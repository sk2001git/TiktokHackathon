...
type VerificationResults = Record<string, Partial<ComplianceResult> & { loading?: boolean; requirement_summary?: string; actionable_verb?: string }>;
...
export function UserView({ userId }: UserViewProps) {
  ...
  const handleVerify = async () => {
    ...
    // Set initial loading state for all selected criteria at once.
    setVerificationResults(prev => {
        const newResults = { ...prev };
        selectedCriterionIds.forEach(id => {
            const criterionData = criteria.find(c => c.criterion_id === id);
            newResults[id] = { ...criterionData, loading: true };
        });
        return newResults;
    });

    // Fire off all verification requests and update the UI as each one finishes.
    const verificationPromises = selectedCriterionIds.map(criterionId => 
      ...
      .then(result => {
        // On success, update the state for this specific criterion.
        setVerificationResults(prev => ({
          ...prev,
          [criterionId]: { ...result, loading: false },
        }));
      })
      .catch(error => {
        // On failure, update the state for this specific criterion with an error.
        toast(`Verification for ${criterionId.slice(0,8)}... failed: ${error.message}`);
        setVerificationResults(prev => ({
          ...prev,
          [criterionId]: { 
            ...prev[criterionId], // Keep existing data like summary
    ...
    // Wait for all promises to be settled (either resolved or rejected) 
    // before turning off the main "verifying" button spinner.
