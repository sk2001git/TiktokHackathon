...
class DifyClient:
    def __init__(self, extract_key: str, verify_key: str, base_url: str):
        ...
        # --- START: URL FIX ---
        # Ensure the base URL includes the /v1 API path prefix
    ...
    async def _run_workflow(self, api_key: str, payload: Dict[str, Any]) -> Any:
        ...
        # Make sure there is an API key before making the request
        if not api_key:
            raise RuntimeError("Dify API key is missing. Please check your DIFY_EXTRACT_API_KEY or DIFY_API_KEY settings.")
        ...
        async with httpx.AsyncClient(timeout=120.0) as client:
            try:
                ...
                body = e.response.text if e.response.text else "[No response body]"
                error_detail = (
                    f"Dify API request failed with status code {e.response.status_code}.\n"
                    f"  - URL: {e.request.method} {e.request.url}\n"
                    f"  - Response: {body}"
                )
                raise RuntimeError(error_detail) from e
            except httpx.RequestError as e:
                raise RuntimeError(f"Dify request failed due to a network error: {e}") from e
    ...
    async def run_verification_workflow(self, criterion: Criterion, document_text: str) -> Dict[str, Any]:
        """Runs the verification workflow and returns the raw dictionary from the AI."""
        payload = {
            "inputs": {
        ...
        # Return the raw dictionary. The calling function will be responsible
        # for constructing the Pydantic model, which is more robust.
        return data
        # --- END: ROBUSTNESS FIX ---
